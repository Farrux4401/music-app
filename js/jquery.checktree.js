...
(function (jQuery) {
  jQuery.fn.checkTree = function (settings) {
    ...
    $lis.not(":has(.arrow)").each(function () {
      ...
      jQuery(this).prepend(
        '<div class="checktree arrow"></div><div class="checktree checkbox"></div>'
      );
    });
    ...
    */
    $tree
      ...
      .addClass(function () {
        return $(this).siblings("ul.expanded").length === 1
          ? "expanded"
          : "collapsed";
      });
    ...
    */
    $checkboxes.on("refresh", function () {
      ...
      var any_checked =
        $this.siblings("ul:first").find(":checkbox:checked:first").length == 1;
      var any_unchecked =
        $this.siblings("ul:first").find(":checkbox:not(:checked):first")
          .length == 1;

      if (any_checked) {
        $this.prop("checked", true);
        if (any_unchecked) {
          $checkbox.addClass("half_checked").removeClass("checked");
          if (settings.onHalfCheck) {
            settings.onHalfCheck($this.parent());
          }
        } else {
          $checkbox.addClass("checked").removeClass("half_checked");
        }
      } else {
